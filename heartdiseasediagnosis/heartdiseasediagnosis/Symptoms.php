...
$query_edit_s = "SELECT id, symptom FROM disease";
$edit_s = mysql_query($query_edit_s, $config) or die(mysql_error());
$row_edit_s = mysql_fetch_assoc($edit_s);
$totalRows_edit_s = mysql_num_rows($edit_s);
?>
<!DOCTYPE html>
...
<table border="0" class="a">
  <tr>
    <td colspan="2" align="center">Symptoms</td>
  </tr>
  <?php do { ?>
    <tr>
      <td width="527"><?php echo $row_edit_s['symptom']; ?></td>
      <td width="114"><form action="Symptoms_modify.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
        <label for="id"></label>
        <input type="hidden" value="<?php echo $row_edit_s['id']; ?>" name="id" id="id" />
        <input type="submit" name="Modify" class="art-button" id="Modify" value="Modify" />
      </form></td>
    </tr>
    <?php } while ($row_edit_s = mysql_fetch_assoc($edit_s)); ?>
</table>
